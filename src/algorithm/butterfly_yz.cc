...
ncclTopoButterfly_yz::ncclTopoButterfly_yz(struct ncclComm *comm): ncclTopoBase(NCCL_ALGO_BUTTERFLY, comm, ncclParamCrossNic(), 0) {}
...
ncclResult_t ncclTopoButterfly_yz::transportSetup() {
  ...
  for (int c=0; c<comm->nChannels; c++) {
	...
	if (channel->butterfly_yz.lastoneCompressed == 0) {
	  NCCLCHECK(ncclTransportP2pConnect(comm, channel, channel->butterfly_yz.peerCount, channel->butterfly_yz.peerRanks, channel->butterfly_yz.peerCount, channel->butterfly_yz.peerRanks));
	}
	else {
	  NCCLCHECK(ncclTransportP2pConnect(comm, channel, channel->butterfly_yz.peerCount - 1, channel->butterfly_yz.peerRanks, channel->butterfly_yz.peerCount - 1, channel->butterfly_yz.peerRanks));
	}
...
ncclResult_t ncclEnqueueButterfly_yz::proxySaveColl(struct ncclProxyArgs *args, struct ncclInfo* info) const {
  ...
  if (pattern == ncclPatternButterfly) {
	...
	struct ncclButterfly_YZ* butterfly = &args->channel->butterfly_yz;
