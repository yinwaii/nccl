...
static ncclResult_t devCommSetup(ncclComm_t comm) {
  ...
  for (int r=0; r<comm->p2pnChannels; r++) {
    ...
    NCCLCHECK(ncclCudaMemcpy(comm->channels[r].butterfly.devPeerRanks, comm->channels[r].butterfly.peerRanks, log2i(comm->nRanks-1)+1));
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, ncclUniqueId* commId) {
  ...
  NCCLCHECK(ncclTopoInit(algos, tmpNnodes));
  ...
  {
    auto *ncclTree = dynamic_cast<ncclTopoTree *>(algos[NCCL_ALGO_TREE].get());
