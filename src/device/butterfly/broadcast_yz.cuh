#ifndef __BUTTERFLY_YZ_BROADCAST_H__
#define __BUTTERFLY_YZ_BROADCAST_H__
...
//   __device__ void runButterfly_yz_br(ncclWorkElem *args) {
//   }
}

template<class RedOp, typename T, int UNROLL>
class ncclFunction<ncclFuncBroadcast, NCCL_ALGO_BUTTERFLY_YZ, NCCL_PROTO_SIMPLE, RedOp, T, UNROLL> {
  public:
  ...
  if (recvPeerRank != -1) {
	Primitives<T, RedOp, FanSymmetric<1>, 0, ProtoSimple<BROADCAST_CHUNKSTEPS/BROADCAST_SLICESTEPS, BROADCAST_SLICESTEPS, UNROLL>>
        prims(tid, args->nThreads, &(recvPeerRank), &(recvPeerRank), thisOutput, channel, comm, 0);
  ...
  for (int p = 0; p < sendPeerCount; p++) {
      Primitives<T, RedOp, FanSymmetric<1>, 0, ProtoSimple<BROADCAST_CHUNKSTEPS/BROADCAST_SLICESTEPS, BROADCAST_SLICESTEPS, UNROLL>>
        prims(tid, args->nThreads, &(sendPeerRanks[p]), &(sendPeerRanks[p]), thisOutput, channel, comm, (p + 1) * 2);
...
template<class RedOp, typename T, int UNROLL>
class ncclFunction<ncclFuncBroadcast, NCCL_ALGO_BUTTERFLY_YZ, NCCL_PROTO_LL, RedOp, T, UNROLL> {
  public:
    __device__ void run(struct ncclWorkElem* args) {\
    }
};
...
template<class RedOp, typename T, int UNROLL>
class ncclFunction<ncclFuncBroadcast, NCCL_ALGO_BUTTERFLY_YZ, NCCL_PROTO_LL128, RedOp, T, UNROLL> {
  public:
    __device__ void run(struct ncclWorkElem* args) {\
    }
};
