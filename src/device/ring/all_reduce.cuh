...
class ncclFunction<ncclCollAllReduce, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE, FUNC, T, UNROLL> {
  public:
...
template<class FUNC, typename T, int UNROLL>
class ncclFunction<ncclCollAllReduce, NCCL_ALGO_RING, NCCL_PROTO_LL, FUNC, T, UNROLL> {
  public:
  __device__ void run(struct CollectiveArgs* args) {
    const int tid = threadIdx.x;
...
template<class FUNC, typename T, int UNROLL>
class ncclFunction<ncclCollAllReduce, NCCL_ALGO_RING, NCCL_PROTO_LL128, FUNC, T, UNROLL> {
  public:
  __device__ void run(struct CollectiveArgs* args) {
    const int tid = threadIdx.x;
    const int nthreads = args->coll.nThreads;
