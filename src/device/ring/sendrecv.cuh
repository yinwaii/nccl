...
class ncclFunction<ncclCollSendRecv, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE, RedOp, T, UNROLL> {
  public:
    ...
    if (args->p2p.delta == 0) {
      if (tid < nthreads && sendbuff != recvbuff) {
        ...
        for (size_t offset=0; offset<args->p2p.sendCount; offset += blockSize) {
          ...
          ReduceOrCopyMulti<UNROLL, RedOp, T, 1, 1, 1, 1>(tid, nthreads, 1, &sendbuff, 1, &recvbuff, blockSize);
    ...
    int peerNone[2] = {-1,-1};
    using Proto = ProtoSimple<1, 1, UNROLL>;
    ...
    if (tid < nthreadsSplit + WARP_SIZE ) {
      ...
      Primitives<T, RedOp, FanAsymmetric<2, 1>, 1, Proto>
        prims(tid, nthreadsSplit, peerNone, &peer, recvbuff, channel, comm);
      ...
      Primitives<T, RedOp, FanAsymmetric<1, 2>, 1, Proto>
        prims(tid-nthreadsSplit-WARP_SIZE, nthreads-nthreadsSplit, &peer, peerNone, recvbuff, channel, comm);
