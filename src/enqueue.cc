...
#define NCCL_FUNC5(func, algo, redop, dtype)                     \
  (void*)WITH_COMMA(NCCL_KERN_NAME(func, algo, LL, redop, dtype)) \
  (void*)WITH_COMMA(NCCL_KERN_NAME(func, algo, LL, redop, dtype)) \
  (void*)WITH_COMMA(NCCL_KERN_NAME(func, algo, LL, redop, dtype))

#define NCCL_FUNC5_ELE(algo, func, redop, dtype)  \
  NCCL_FUNC5(func, algo, redop, dtype)

#define NCCL_FUNC4(func, redop, type) \
  MAP_FOR_ALGOS(NCCL_FUNC5_ELE, func, redop, type)
...
static ncclResult_t computeColl(struct ncclInfo* info /* input */, struct ncclWorkElem* work, struct ncclProxyArgs* proxyArgs /* output */) {
  ...
  ncclAlgos[info->algorithm]->enqueueSlice(info, &sliceInfo, work);
  ...
  // round up
  proxyArgs->recvbytes = sliceInfo.stepSize*proxyArgs->sliceSteps;
...
ncclResult_t ncclSaveKernel(struct ncclInfo* info) {
  ...
  for (int bid=0; bid<nChannels*nSubChannels; bid++) {
    ...
    if (proxyArgs.nsteps) NCCLCHECK(ncclAlgos[info->algorithm]->proxySaveColl(&proxyArgs, info));
