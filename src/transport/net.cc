...
ncclResult_t netSendProxy(struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    if (args->head == args->end) {
      ...
      //switch - lyz 
      //this may not be necessary as the proxy will soon be removed
      args->switch_state = 0;
    }
  }
  return ncclSuccess;
...
ncclResult_t netRecvProxy(struct ncclProxyArgs* args) {
  ...
  //printf("Algo %d Opcount %d: Calling netRecv\n",args->opCount,args->opCount);
  if (args->state == ncclProxyOpReady) {
    // Round to next multiple of sliceSteps
    resources->step = ROUNDUP(resources->step, args->chunkSteps);
    args->head = resources->step;
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    char* localBuff = args->connector->conn.buffs[p];
    void* mhandle = *(resources->mhandlesProto[p]);
    //printf("head%llu tail%llu end%llu steps:%llu\n",args->head,args->tail,args->end,args->nsteps);
    if (args->head < args->end) {
